# OfflinePapi

-------------------------------------------------------------------------------

[**🌎中文文档**](README-CH.md)

-------------------------------------------------------------------------------

A somewhat useful offline variable plugin

If this plugin helps you, please support the author by clicking the Star in the upper right corner～

Usage tutorial: [Click to view](https://ricedoc.handyplus.cn/wiki/OfflinePapi/README/)  
Purchase address: [Click to view](https://afdian.com/item/250e635cce3611eda09152540025c377)

Development version download channels:

[GITHUB ACTIONS](https://github.com/handy-git/OfflinePapi/actions)

After purchase, you will receive after-sales service. The author will provide usage guidance. If not purchased, please refer to the wiki for usage instructions.

## Usage Statistics

![](https://bstats.org/signatures/bukkit/OfflinePapi.svg)

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=handy-git/OfflinePapi&type=Date)](https://star-history.com/#handy-git/OfflinePapi&Date)